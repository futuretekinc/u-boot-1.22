 ...
 *
 * $Revision: 13571 $
 * $Date: 2010-10-25 16:04:01 +0800 (星期一, 25 十月 2010) $
 *
...
@comm
    The API can turn on/off leds of dedicated port while indicated information configuration of LED group is set to force mode.
 */
ret_t rtl8370_getAsicForceGroupLed(uint32 group, uint32* mode)
{
    ret_t retVal;
	uint32 stat, tmpstat;
    uint32 port;
	
	for(port=0;port<RTL8370_LEDPORTMAX;port++)
	{
        retVal = rtl8370_getAsicForceLed(port, group, &tmpstat);
        if( retVal !=  RT_ERR_OK)
            return retVal;
        if (stat != tmpstat)
        {
