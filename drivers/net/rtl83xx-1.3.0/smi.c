...
static inline void MDC_MDIO_WRITE(unsigned int len, unsigned int devid,
				 unsigned int reg, unsigned int data)
{
	g2_phy_write(devid, reg, data);
}

static inline void MDC_MDIO_READ(unsigned int len, unsigned int devid,
				 unsigned int reg, unsigned int *pdata)
{
	*pdata = g2_phy_read(devid, reg);
}
...
#endif /* End of #ifndef MDC_MDIO_OPERATION */
