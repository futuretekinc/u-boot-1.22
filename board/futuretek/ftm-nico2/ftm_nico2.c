...
void power_hold(void)
{
	...
	reg_val = __raw_readl(GLOBAL_GPIO_MUX_1);
	reg_val |= (1 << 25) | (1 << 26);
	__raw_writel(reg_val, GLOBAL_GPIO_MUX_1);

	reg_val = __raw_readl(PER_GPIO1_CFG);
	reg_val = reg_val & ~((1 << 25) | (1 << 26));
	__raw_writel(reg_val, PER_GPIO1_CFG);

	reg_val = __raw_readl(PER_GPIO1_OUT);
	reg_val = reg_val | (1 << 25);
	__raw_writel(reg_val, PER_GPIO1_OUT);

	reg_val = __raw_readl(PER_GPIO1_OUT);
	reg_val = reg_val | (1 << 26);
	__raw_writel(reg_val, PER_GPIO1_OUT);
