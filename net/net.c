...
#if defined(CONFIG_CMD_HTTP)
static void HttpStart(void);
#endif
...
#if defined(CONFIG_CMD_HTTP)
int
NetSendIPPacket(uchar *ether, IPaddr_t dest, uchar proto_id, uint len)
{
	uchar 	*pkt;
	IP_t	*ip;
	uint	total_len;
	...
	if (memcmp(ether, NetEtherNullAddr, 6) == 0) {
		...
		ip->ip_tos   = 0;
		ip->ip_len   = htons(IP_HDR_SIZE_NO_UDP + len);
		...
		ip->ip_sum   = ~NetCksum(ether + 14, 20);;
	...
	memcpy(pkt+20, ether+34, len);
	total_len = (pkt - NetTxPacket) + IP_HDR_SIZE_NO_UDP + len;
	(void) eth_send(NetTxPacket, total_len);
...
#if defined(CONFIG_CMD_HTTP)
static void
HttpHandler (uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
	return;
}
...
NetReceive(volatile uchar * inpkt, int len)
{
	...
	case PROT_IP:
		...
		if (ip->ip_p == IPPROTO_ICMP) {
			...
			/* don't consider IP option */
			tcp_rcve((uchar *)inpkt, len-20);
			return;
#endif
