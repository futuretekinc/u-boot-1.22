...
LINUX_DIR=$(CURDIR)/../linux-2.6.36

PLATFORM_INCLUDE = $(LINUX_DIR)/arch/arm/mach-goldengate/include/mach
...
export	LANG HOSTARCH HOSTOS SHELL PLATFORM_INCLUDE
...
ifdef CONFIG_CS75XX_MAC_TO_EXT_SWITCH
LIBS += drivers/net/rtl83xx-1.3.0/librtl83xx.a
endif
...
ALL += $(obj)u-boot.srec $(obj)u-boot.bin $(obj)System.map $(U_BOOT_NAND) $(U_BOOT_ONENAND) $(obj)bootloader.bin $(obj)u-boot-env.bin
...
$(obj)u-boot-env.sh: $(obj)tools/mk_env/$(BOARD)-env.tmpl
		perl $(obj)tools/mk_env/expand-tmpl.pl < $(obj)tools/mk_env/$(BOARD)-env.tmpl > $(obj)u-boot-env.sh

$(obj)u-boot-env.bin: $(obj)u-boot-env.sh
		dd if=/dev/zero of=$(obj)u-boot-env.bin bs=1 count=131072
		$(obj)tools/flashbin-setenv/uboot-flashbin-setenv \
			-binfile $(obj)u-boot-env.bin \
			-envoffset 0 \
			-envsize 0x20000 \
			-setenv $(obj)u-boot-env.sh

$(obj)bootloader.bin: $(obj)u-boot.bin $(obj)tools/post.bin
		$(obj)tools/gen_post_uboot_bin -p $(obj)bootstrap/post.bin -u $(obj)u-boot.bin -o bootloader.bin

$(obj)tools/post.bin: 
		$(MAKE) -C $(obj)bootstrap
...
TAG_SUBDIRS += include
TAG_SUBDIRS += lib_generic lib_openssl board/$(BOARDDIR)
TAG_SUBDIRS += cpu/$(CPU)
...
goldengate_config:		unconfig
	$(MKCONFIG) -t $(CS_PLATFORM_TYPE) $(@:_config=) arm arm_cortexa9 goldengate NULL goldengate

ftm_100s_config:		unconfig
	$(MKCONFIG) -t FTM_100S $(@:_config=) arm arm_cortexa9 ftm-100s futuretek goldengate 

ftg_1000_config:		unconfig
	$(MKCONFIG) -t FTG_1000 $(@:_config=) arm arm_cortexa9 ftg-1000 futuretek goldengate 

ftm_nico1_config:		unconfig
	$(MKCONFIG) -t FTM_NICO1 $(@:_config=) arm arm_cortexa9 ftm-nico1 futuretek goldengate 

ftm_nico2_config:		unconfig
	$(MKCONFIG) -t FTM_NICO2 $(@:_config=) arm arm_cortexa9 ftm-nico2 futuretek goldengate 

ftw_1000_config:		unconfig
	$(MKCONFIG) -t FTW_1000 $(@:_config=) arm arm_cortexa9 ftw-1000 futuretek goldengate 

ftw_1000_v2_config:		unconfig
	$(MKCONFIG) -t FTW_1000_V2 $(@:_config=) arm arm_cortexa9 ftw-1000 futuretek goldengate 
